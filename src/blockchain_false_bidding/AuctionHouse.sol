...
contract AuctionHouse {
    ...
    receive() external payable {
        if ((uint64(msg.value) >= 2 * topBidder().bid) && (msg.sender != topBidder().addr) && (!blacklisted[msg.sender])&& (_isPayable(msg.sender))) {
            _newBidder(msg.sender, uint64(msg.value)); //consigo me tornar o topBidder, após a função executa o _isPayable() eu posso gastar quase todo o seu gás para que ele não consiga executar a função de baixo
            timeout += YEAR;
        }
    }
    ...
    function withdrawFromAuction() public topBidderOnly {
        ...
        (bool success,) = payable(withdrawer.addr).call{value: withdrawer.bid / 2}(""); //quando chamar eu mando de volta o valor para me tornar topbidder novamente
        if (success) {
