...
contract Lockers {
    ...
    function sellItem(string calldata name, string calldata password) external {
        ...
        (bool success,) = usernameToWallet[prevOwner].call{value: price[_item.rarity]}(""); //reentrancy
        require(success);
        delete items[index];
    }

    function getLocker(string calldata username, string calldata password) external { //Aqui a gente altera a senha de qualquer user que não seja o Vendor
